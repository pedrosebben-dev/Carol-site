...
/** Botão flutuante que só aparece depois que o usuário rolou o hero. */
const { whatsappLink } = useSite()
const { scrollY } = useScrollState()

const visible = computed(() => scrollY.value > 600)
</script>
...
<template>
  <ClientOnly>
    <a
      ...
      class="group fixed bottom-6 right-6 z-[130] flex items-center gap-3 rounded-full bg-brass py-3.5 pl-4 pr-5 text-void shadow-2xl shadow-void/50 transition-all duration-700 ease-out-expo"
